...
src-core ──→ src-plugin-types ─────────────────────────────────────────────┤
             src-plugin-types ──→ src-desktop-types ──→ src-editor ───────→ src-desktop-core ──→ src-desktop-native (std)
                                  src-desktop-types ──→ src-desktop-layout ──→                ──→ src-desktop         (std+Tauri)
...
注: src-plugin-types が src-core に依存するのは to_plugin_events(&[Event<'_>]) → Vec<PluginEvent>
...
|---------|--------|------|
| `src-core` | ✅ | 変更なし |
| `src-plugin-types` | ✅ | `serde_json = { default-features=false, features=["alloc"] }` に変更が必要。`serde_json::Value` は alloc モードでも利用可能。`src-core` に依存（`to_plugin_events` のため） |
