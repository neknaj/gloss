...
> **ステータス:** Draft v4（spec review v1〜v3 反映済み）
...
```
src-core ──────────────────────────────────────────────────────────────────────────────────┐
src-core ──→ src-plugin-types(*) ──────────────────────────────────────────────────────────┤
             src-plugin-types ──→ src-desktop-types ──→ src-editor ──→ src-desktop-core ─→ src-desktop-native (std)
             src-plugin-types ──→ src-desktop-types ──→ src-desktop-layout              ──→ src-desktop         (std+Tauri)
             src-plugin-types ──→ src-desktop-types ──────────────→ src-desktop-core   ──→ src-desktop-wasm    (wasm32)
                                                                                        ──→ src-cli             (std)
...
(*) src-plugin-types が src-core に依存するのは to_plugin_events(&[Event<'_>]) → Vec<PluginEvent>
    変換関数のため（Event 型が src-core で定義されるから）

重要: src-desktop-layout と src-desktop-core は独立した兄弟クレートである。
どちらも src-desktop-types に依存するが、互いには依存しない。
シェル層（src-desktop / src-desktop-wasm / src-cli）が両クレートを直接消費する。
```
...
|---------|--------|------|
| `src-core` | ✅ | 変更なし |
| `src-plugin-types` | ✅ | `serde_json = { default-features=false, features=["alloc"] }` に変更が必要。`serde_json::Value` は alloc モードで利用可能（`CodeHighlightInput`・`CardLinkInput`・`LintRuleInput`・`FrontMatterInput` の既存フィールドも問題なし）。`src-core` に依存（`to_plugin_events` のため） |
...
### 3.1b 基本プリミティブ型（`src-desktop-types` 内）
...
pub struct KeyEvent {
    pub key:   KeyCode,
    pub mods:  Modifiers,
    pub text:  Option<String>,  // 印刷可能文字（IME 経由でないもの）
}
pub struct Modifiers { pub ctrl: bool, pub shift: bool, pub alt: bool, pub meta: bool }
pub enum KeyCode {
    Char(char), Enter, Backspace, Delete, Escape, Tab,
    ArrowUp, ArrowDown, ArrowLeft, ArrowRight,
    Home, End, PageUp, PageDown, F(u8),
}
...
pub enum MouseKind   { Press, Release, Move, Scroll { delta_x: f32, delta_y: f32 } }
pub enum MouseButton { Left, Right, Middle }

// ── エディタ選択 ──
pub struct Selection { pub anchor: usize, pub active: usize }  // バイト位置

// ── DrawCmd 補助型 ──
pub struct CursorDisplay  { pub line: u32, pub visual_col: u32, pub blink: bool }
pub struct SelectionDisplay { pub ranges: Vec<(u32, u32, u32, u32)> } // (line, col_start, line_end, col_end)
pub struct PreeditDraw    { pub text: String, pub underline_range: Option<(usize, usize)> }
...
pub struct WarningInfo {
    pub code:    String,
    pub message: String,
    pub line:    Option<u32>,  // 表示用アダプタ型。PluginWarning.line (u32) から変換する
}
pub enum DialogKind {
    OpenFile,
...
pub struct HtmlPatch {
    pub block_id: u64,       // fnv1a ブロックハッシュ
    pub html:     String,    // 新しい HTML フラグメント
}
...
// src-plugin-types は src-core に依存して to_plugin_events を提供する
// 変換関数シグネチャ（src-plugin-types 内に定義）:
// pub fn to_plugin_events<'a>(events: &'a [src_core::Event<'a>]) -> Vec<PluginEvent>
...
// target_arch = "wasm32" ゲートで src-desktop-wasm のプロダクションコードからも使用可能

// 注: src-desktop-core のテストから NoopPluginHost を使うには dev-dependencies に
// src-desktop-types = { features = ["test-utils"] } を追加する必要がある。
// cfg(test) は依存クレートには伝播しないため、feature フラグが必要。
```
...
impl Highlighter {
    ...
    /// TextSpan.text は String（所有型）なのでライフタイムパラメータなし
...
impl<Fs: FileSystem, Ph: PluginHost> AppCore<Fs, Ph> {
    ...
    // ── ドキュメント管理 ──
    /// 同期的に Fs::read() を呼ぶ（CLI・テスト・MemoryVfs 用途）
    /// Tauri シェルは ReadFile AppCmd → FileLoaded → open_bytes() の非同期パスを使う
    pub fn open_file(&mut self, path: &VfsPath)
        -> Result<(DocId, EditorViewModel), FsError>;
    /// 非同期読み込み完了後にシェルが呼ぶ（FileLoaded イベントのハンドラから）
    ...
    /// fields は src_plugin_types::PluginFrontMatterField（所有型、ライフタイムなし）
    pub fn apply_front_matter(&mut self, doc_id: DocId, fields: &[PluginFrontMatterField]);
...
buffer.as_str()
  ...
  → to_plugin_events(&events)          ← src-plugin-types に移行した変換関数
                                         （現在は src-plugin/convert.rs にあるが移行する）
...
`src-desktop-core` は `src-plugin-types` に依存（`to_plugin_events`・`PluginEvent`・`PluginFrontMatterField` のため）。
**移行作業：** `src-plugin/src/convert.rs` の `to_plugin_events()` を `src-plugin-types` に移動する
（`src-plugin-types` は既に `src-core` に依存する形に変更される）。
`src-plugin::convert` のオリジナルはその後削除または `src-plugin-types` を再エクスポートする。
...
// Arc を使う（Mutex は Clone 不可のため spawn_blocking のクロージャへの移動に Arc が必要）
...
async fn dispatch(
    ...
    let draw_cmds = tokio::task::spawn_blocking({
        let state: Arc<Mutex<AppState>> = state.inner().clone();  // Arc のクローン（Mutex ではない）
        move || {
...
差分更新は `AppCore::render_diff()` が担当する（シェル層が直接 `split_source_blocks()` を呼ぶのではない）。
`AppCore::render_diff()` は内部で `src-core::split_source_blocks()` と `fnv1a()`（no_std 実装済み）を使って
変更ブロックを検出し `Vec<HtmlPatch>` を返す。シェル層はこの結果を `PreviewPatch` DrawCmd に変換するだけ。
...
| 対象 | 最終方針 |
|------|---------|
| `GlossPluginHost` | `run_lint_rule` シグネチャを `&[Event<'_>]` → `&[PluginEvent]` に変更。内部 `to_plugin_events()` 呼び出しを削除（呼び出し元 AppCore が変換済みを渡すため）。`src-desktop-native` で `PluginHost` トレイトを実装 |
